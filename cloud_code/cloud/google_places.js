...
exports.locationSearch = function(options, callbackSuccess, callbackError) {
	...
	var placeType = options.type;
	var placeName = options.placeName;
	if (!radius) {
		radius = 1000;
	}
	// We create the Url for the google place free search
	...
	if (placeType != null) {
		locationSearchUrl = locationSearchUrl +"&types="+placeType;
	}	
	if (placeName != null) {
		//locationSearchUrl = locationSearchUrl +"&name="+placeName+"&keyword="+placeName;
		locationSearchUrl = locationSearchUrl +"&keyword="+placeName;
	}
...
exports.simplePlaceDetail = function(countryCode, googleReference, callbackSuccess, callbackError) {
	var languageCode = _defaultCountryCode;
	if (!countryCode){
		languageCode = _defaultCountryCode;
	}
