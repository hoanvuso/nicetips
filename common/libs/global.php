...
/**
 * return www url
 * @return string url to www folder
 */
function www() {
    return WWW_URL;
}
 ...
 * return integer
 */
function returnIntegerDate($date, $time) {
    //tach chuoi start time
    if ($time != '') {
        $array = explode(':', $time);
        $stringDate = $date . ' ' . $array[0] . ':' . $array[1];
    } else {
        $stringDate = $date;
    }
 ...
 * return string
 */
function stringDateAgo($date) {
    // thoi gian con lai giua ngay hien tai va ngay can so sanh
    $countTime = time() - $date;
    $year = (int) ($countTime / (365 * 24 * 3600));

    $countMonth = (int) ($countTime % (365 * 24 * 3600));


    $month = (int) ($countMonth / (30 * 24 * 3600));
    $countDay = (int) ($countMonth % (30 * 24 * 3600));
    $day = (int) ($countDay / (24 * 3600));
    $countHours = (int) ($countDay % (24 * 3600));
    $hours = (int) ($countHours / (3600));
    $minutes = (int) ($countHours % (3600));
    $stringTime = '';
    if ($year > 0) {
        if ($year == 1) {
            $stringTime .= $year . ' year ';
        } else {
            $stringTime .= $year . ' years ';
        }
    }
    if ($month > 0) {
        if ($month == 1) {
            $stringTime .= $month . ' month ';
        } else {
            $stringTime .= $month . ' months ';
        }
    }
    if ($day > 0) {
        if ($day == 1) {
    ...
    if ($month > 0 || $day > 0 || $hours > 0) {
        $stringTime .= ' ago';
    } elseif ($minutes > 0) {
        $stringTime = (int) ($minutes / 60) . ' minutes ago';
    }
    return $stringTime;
}
...
/**
 *
 * @param type $pages Cpagenation
 */
function getPageination($pages) {
    $count = $pages->getItemCount(); //tong item
    $total = $pages->getPageSize(); // so item trong trang page=10;
    $currentPage = $pages->getCurrentPage(); //trang hien tai
    $totalPage = $pages->getPageCount(); //so trang
    $endPage = $count % $total;
    if ($currentPage === ($totalPage - 1)) {
        if ($endPage === 0) {
            $endPage = $total;
        }
        $end = $endPage + ($currentPage * $total);
    } else {
        $end = $total + $currentPage * $total;
    }
    $start = $currentPage * $total + 1;


    return array('start' => $start, 'end' => $end);
}

function doDownloadFile($filename) {
    if (file_exists($filename)) {
        //header('Location: '.$url_filename);
